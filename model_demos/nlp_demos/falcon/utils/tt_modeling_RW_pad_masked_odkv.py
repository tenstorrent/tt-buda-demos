...
class PaddedAttentionTT(nn.Module):
    ...
    def make_pad_weights(self, make_kvs=False):
        ...
        """
        self.wq.weight.data = self.mask_weight(
            self.wq.weight.data,
            self.original_hidden_size,
            self.original_hidden_size,
        )
        self.dense.weight.data = self.mask_weight(
            self.dense.weight.data,
            self.original_hidden_size,
            self.original_hidden_size,
        )
        if make_kvs:
    ...
    def forward(
        ...
        if self.user_rows > 1:
            return self.attn_user_batch(
                ...
                kv_read_mask,
                kv_write_mask,
            )  # , num_tokens, kv_read_mask, kv_write_mask)
        else:
            return self.attn(
                hidden_states,
                attention_mask,
    ...
    def split_qkv_weights(self):
        ...
        self.wk.weight = nn.Parameter(
            self.query_key_value.weight[self.hidden_size : self.hidden_size + self.head_dim].clone()
        )
...
class PaddedMLP(nn.Module):
    ...
    def make_pad_weights(self):
        ...
        """
        self.dense_4h_to_h.weight.data = self.mask_weight(
            self.dense_4h_to_h.weight.data,
            self.original_hidden_size,
            self.original_hidden_size * 4,
        )
        self.dense_h_to_4h.weight.data = self.mask_weight(
            self.dense_h_to_4h.weight.data,
            self.original_hidden_size * 4,
            self.original_hidden_size,
        )
...
class PaddedLayerNorm(nn.Module):
    ...
    def __init__(
        ...
        self.mask[-pad_amt:] = 0
        self.scale = normalized_shape[0] / (
            normalized_shape[0] - pad_amt
        )  # this is the scale factor for the mean, n_p/n
    ...
    def forward(self, input, masked_input=False):
        ...
        if not masked_input:
            x_p = (
                input * self.mask
            )  # this masks out the padded values, if we can assure input is already 0 padded, we can remove this
        else:
            x_p = input
...
class RWModel(RWPreTrainedModel):
    ...
    def main_forward_part(
        ...
        **deprecated_arguments,
        ...
        padded_hidden = (
            self.config.hidden_size + (self.config.hidden_size // self.config.n_head) * self.config.padded_heads
        )
