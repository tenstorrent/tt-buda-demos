...
@pytest.mark.opt
def test_opt_causal_lm_pytorch(clear_pybuda, test_device, variant):
    run_opt_casual_lm(variant)


@pytest.mark.parametrize("variant", variants, ids=variants)
@pytest.mark.opt
def test_opt_question_answering_pytorch(clear_pybuda, test_device, variant):
    run_opt_question_answering(variant)


@pytest.mark.parametrize("variant", variants, ids=variants)
@pytest.mark.opt
def test_opt_sequence_classification_pytorch(clear_pybuda, test_device, variant):
