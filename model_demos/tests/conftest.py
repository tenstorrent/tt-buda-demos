        ...
        if name == "Golden":
            if device_config and DEVICE_CONFIG_TO_BACKEND_DEVICE_TYPE.get(device_config, None):
                return TestDevice(
                    ...
                    arch=DEVICE_CONFIG_TO_BACKEND_DEVICE_TYPE[device_config],
                    devmode=devmode,
                    tti_path=tti_path,
                )
            elif "GOLDEN_WORMHOLE_B0" in os.environ:
                return TestDevice(
                    devtype=BackendType.Golden, arch=BackendDevice.Wormhole_B0, devmode=devmode, tti_path=tti_path
                )
            elif "PYBUDA_GOLDEN_BLACKHOLE" in os.environ:
                return TestDevice(
                    devtype=BackendType.Golden, arch=BackendDevice.Blackhole, devmode=devmode, tti_path=tti_path
                )
            return TestDevice(
                devtype=BackendType.Golden, arch=BackendDevice.Grayskull, devmode=devmode, tti_path=tti_path
            )
        if name == "Model":
            return TestDevice(
                devtype=BackendType.Model, arch=BackendDevice.Grayskull, devmode=devmode, tti_path=tti_path
            )
        if name == "Versim":
            ...
            if versim_arch_name is not None:
                versim_backend_device = BackendDevice.from_string(versim_arch_name)
            return TestDevice(
                devtype=BackendType.Versim, arch=versim_backend_device, devmode=devmode, tti_path=tti_path
            )
        if name == "Emulation":
            ...
            if emulation_arch_name is not None:
                emulation_backend_device = BackendDevice.from_string(emulation_arch_name)
            return TestDevice(
                devtype=BackendType.Emulation, arch=emulation_backend_device, devmode=devmode, tti_path=tti_path
            )
        if name == "Grayskull":
            return TestDevice(
                devtype=BackendType.Silicon, arch=BackendDevice.Grayskull, devmode=devmode, tti_path=tti_path
            )
        if name == "Wormhole_B0":
            return TestDevice(
                devtype=BackendType.Silicon, arch=BackendDevice.Wormhole_B0, devmode=devmode, tti_path=tti_path
            )
        if name == "Blackhole":
            return TestDevice(
                devtype=BackendType.Silicon, arch=BackendDevice.Blackhole, devmode=devmode, tti_path=tti_path
            )
        raise RuntimeError("Unknown test device: " + name)

    def is_available(
        self,
        device_list: List[BackendDevice],
        silicon_only: bool,
        no_silicon: bool,
        devtype: Optional[BackendType],
        devmode: DeviceMode,
    ) -> bool:
        """
...
def pytest_addoption(parser):
    ...
    parser.addoption("--batch-size", default=1, type=int, help="Specify batch size for the tests")
    parser.addoption(
        "--batched-run", action="store_true", default=False, help="Enable batched run for batch sizes 2 or more"
    )
...
# This decorator indicates that the function pytest_cmdline_main should be executed as early as possible in the pytest hook processing order.
@pytest.hookimpl(tryfirst=True)
def pytest_cmdline_main(config):
    batch_size = config.getoption("--batch-size")
    batched_run = config.getoption("--batched-run")
    if batched_run and batch_size >= 2:
        print("Batched run enabled with batch size 2 or more.")
        # Enable Env Variable for running on Wormhole_B0_X2 or n300
        os.environ["PYBUDA_N300_DATA_PARALLEL"] = "1"
        print("Batched run enabled with batch size 2 or more. Environment variable set.")
